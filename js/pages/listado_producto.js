...
document.addEventListener('DOMContentLoaded', async function() {
    console.log('DOM listo, esperando productos...');
    ...
    try {
        if (typeof loadProductsFromDB === 'function') {
            ...
            console.log('✓ Productos cargados manualmente');
            console.log('PRODUCTOS_DB:', window.PRODUCTOS_DB);
            initializeProducts();
        } else {
            console.error('loadProductsFromDB no existe');
        }
    } catch (error) {
...
function getAllProducts() {
    if (typeof window.PRODUCTOS_DB === 'undefined') {
        console.error('PRODUCTOS_DB no está definido todavía');
        return {};
    }
    
    const allProducts = {};
    
    if (window.PRODUCTOS_DB.notebooks) {
        Object.assign(allProducts, window.PRODUCTOS_DB.notebooks);
    }
    
    if (window.PRODUCTOS_DB.monitors) {
        Object.assign(allProducts, window.PRODUCTOS_DB.monitors);
    }
...
function openProductModal(productId) {
    window.location.href = `ficha_producto.php?id=${productId}`;
}
