...
function loadProduct(productId) {
  ...
  if (def) {
    ...
    showProductContainer(true);
    startAutoSlide();
    window.history.replaceState({}, '', `ficha_producto.php?id=notebook1`);
  }
}
...
function updateBreadcrumb(product) {
  ...
  if (cat) {
    if (product.type === 'notebook') {
      ...
      cat.onclick = () => location.href = 'notebooks.php';
    } else {
      cat.textContent = 'Monitores';
      cat.onclick = () => location.href = 'monitores.php';
    }
...
function buyNow() {
  addToCartFromDetail();
  setTimeout(() => { location.href = 'comprar.php'; }, 500);
}
...
function goToProduct(id) {
  stopAutoSlide();
  location.href = `ficha_producto.php?id=${id}`;
}
